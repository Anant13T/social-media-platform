...
def index(request):
    user_object=User.object.get(username=request.user.username)
    ...
    for ids in username_profile:
        profile_lists=Profile.objects.filter(ids_user=ids)
...
def upload(request):
    if request.methhod=='POST':
        ...
        caption=request.FILES.POST['caption']
    ...
    if request.method=='POST':
        ...
        if password==password2:
            if User.objects.filter(email=email).exists():
                ...
                messages.info(request,'Usernam in use')
                return redirect('signup')
            else:
